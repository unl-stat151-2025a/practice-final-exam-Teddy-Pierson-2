...
2. How many of the recorded yarns are machine washable?
    ...
    > The original data type is boolean (True/False). When I take the sum of these values, R converts the booleans to integers (either 0 or 1). NA values can't be added so I first remove or replace them and take the sum of the new integers 
...
n_washable= tmp.machine_washable
```
...
for i in range(ncol):
  n_missing_by_col[i]=sum(yarn.iloc[:,1].isna())
...
common_yarn=common_yarn.loc
[:,['yarn_company_name','yarn_weight_name']]

common_yarn=common_yarn.groupby(['yarn_company_name'
,'yarn_weight_name']).size().reset_index()

common_yarn=common_yarn.rename(columns={0:'n'})

# Create a plot of the brand and yarn weight using a tile geometric object, mapping `n` to the fill color of the tile. If you cannot figure out the tile plot, you can use a scatterplot to make a similar (though less easily readable) plot, mapping point size to `n` instead. 

tmp=common_yarn.pivot(columns='yarn_company_name', index=
'yarn_weight_name',values='n')
sns.heatmap(tmp)
plt.show()
...
descriptors <- yarn$texture_clean |> str_split("[,;]")|> 
  unlist() |> str_trim()
...
all_descriptors=yarntmp.texture_clean.str.split('[,:]',regex=
True).explode()
...
def is_top_descriptor(x):
  ...
  mat=np.array([[i.find(j)!=-1 for j in top_descriptors.index] for i in
  x.values])
...
yarn <- yarn |> mutate(yarn_weight_wpi_num=convert_weight_int
(yarn_weight_wpi)
